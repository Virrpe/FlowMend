#!/usr/bin/env tsx
/**
 * E2E Positive Test #2 (Gate 1)
 * Second run with different metafield key to prove repeatability
 *
 * Usage: npx tsx scripts/e2e-positive-test-2.ts
 */
...
async function main() {
  console.log('🧪 Gate 1 (Test #2): E2E Positive Test with Different Metafield\n');

  const shopId = process.env.SHOPIFY_STORE_DOMAIN!;

  // Test job configuration - REAL WRITE with different key
  const testJob = {
    shopId,
    queryString: 'status:active',
    namespace: 'custom',
    key: 'flowmend_test_2', // Different key from first test
  ...
  await prisma.jobEvent.create({
    data: {
      ...
      message: 'Job created via E2E positive test #2',
    },
  });
  ...
  if (allPassed) {
    console.log('\n✅ Gate 1 Test #2 PASSED - Repeatability confirmed\n');
  } else {
    console.log('\n❌ Gate 1 Test #2 FAILED - Check job events for details\n');
    process.exit(1);
  }
